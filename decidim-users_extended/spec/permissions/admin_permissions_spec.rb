...
describe Decidim::Admin::Permissions do
  ...
  let(:ad_admin) { create :user, :admin, organization: organization, admin: false, ad_role: 'admin' }
  ...
  let(:coordinator) { create :process_admin, participatory_process: process, ad_role: 'coordinator', admin_terms_accepted_at: Date.current }
  let(:process_admin) { create :process_admin, participatory_process: process, admin_terms_accepted_at: Date.current }
  let(:outside_coordinator) { create :process_admin, ad_role: 'coordinator', participatory_process: other_process, admin_terms_accepted_at: Date.current }
  ...
  # let(:moderator) { create :process_moderator, participatory_process: process, ad_role: 'moderator', admin_terms_accepted_at: Date.current }
  let(:process_valuator) { create :process_valuator, participatory_process: process, admin_terms_accepted_at: Date.current }
  # let(:expert) { create :user, :admin_terms_accepted, ad_role: 'expert', organization: organization }
