...
function compareText(source_compare, field_value, output) {

    // Get element values
    // refSentenceElement = $("#source_comapare").text();
    // mySentenceValue = $("#project_body").val();
    // output = $("#result_comapare");
    // Get element values
    refSentenceElement = source_compare.text();
    mySentenceValue = field_value.val();
    ...
    for (let i = 0; i < mySentenceValueSplitted.length; i++) {
        ...
        if (algoReturn.newWords.includes(currentWord)) {
            myWord.html = "<span class='compare_new'>" + currentWord + "</span>"
            myWord.newWord = true
        }

        else if (algoReturn.shiftedWords.includes(currentWord)) {
            myWord.html = "<span class='compare_shifted'>" + currentWord + "</span>"
            myWord.shifted = true
        }
        else {
            myWord.html = "<span class='compare_normal'>" + currentWord + "</span>"
        }
        myWords.push(myWord)
    }
    ...
    for (let i = 0; i < algoReturn.delWordsIndices.length; i++) {
        ...
        myWord.html = "<span class='compare_del'>" + algoReturn.delWords[i] + "</span>"
        myWord.delWord = true
