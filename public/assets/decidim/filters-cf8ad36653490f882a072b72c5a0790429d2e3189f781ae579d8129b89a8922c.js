      ...
      value: function mountComponent() {
        ...
        if (this.$form.length > 0 && !this.mounted) {
          (function () {
            ...
            var contentContainer = $(_this.$form.closest(".filters").parent().find(".skip").attr("href"));
            if (contentContainer.length === 0) {
              contentContainer = _this.$form.data("remoteFill");
            }
            _this.$form.on("change", "input:not([data-disable-dynamic-change]), select:not([data-disable-dynamic-change])", _this._onFormChange);

            _this.currentFormRequest = null;
            _this.$form.on("ajax:beforeSend", function (e) {
            ...
            _this.$form.on("ajax:success", function () {
              queue -= 1;
            ...
            _this.$form.on("ajax:error", function () {
              queue -= 1;
              if (queue <= 0 && contentContainer.length > 0) {
                contentContainer.removeClass("spinner-container");
              }
            ...
            exports.theCheckBoxesTree.setContainerForm(_this.$form);

            exports.Decidim.History.registerCallback("filters-" + _this.id, function (state) {
