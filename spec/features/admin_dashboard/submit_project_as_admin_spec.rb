...
module Decidim
  RSpec.describe 'submit project as admin', type: %i[feature mailer] do
    ...
    it 'creates project and redirect to show with flash message' do
      ...
      expect(page).to have_content('Projekt przekazano do koordynatora')
      expect(page).to have_current_path(url_expected)

      click_link 'Opublikuj papierowy projekt'

      expect(page).to have_content('Projekt został opublikowany')
      expect(page).to have_current_path(url_expected)

      click_link 'Przekaż do komórki'

      expect(page).to have_current_path("/admin/participatory_processes/#{part_process.slug}/components/#{projects_component.id}/manage/projects/#{Decidim::Projects::Project.last.id}/evaluations/choose_department")

      # there is a bug in test - despite the fact, that there are possible departments to choose,
      # capybara do not see desired option with screenshot / open page
      # select('Dzielnice', from: 'Lista komórek')
      # click_link 'Przekaż'
      # expect(page).to have_content('Projekt został przekazany do komórki')
      # expect(page).to have_current_path("/admin/participatory_processes/#{part_process.slug}/components/#{projects_component.id}/manage")
    end
  end
end
