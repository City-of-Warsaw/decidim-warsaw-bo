...
module Decidim
  RSpec.describe 'submit project as admin with coauthors', type: %i[feature mailer] do
    ...
    let!(:user) do
      create(
        ...
        ad_role: :decidim_bo_bie_admin
      )
    end

    let(:email_coauthor_1) { 'jan_wspolautor_1@localhost' }
    let(:email_coauthor_2) { 'jan_wspolautor_2@localhost' }

    it 'creates project and redirect to show with flash message' do
      sign_in(user)
      visit url_target
      ...
      all('.card-divider.coauthors-unfold-js').each do |element|
        within(element) { find('svg[aria-label="Rozwiń"]').click }
      end

      within('#coauthor-one-js') do
        fill_in 'Imię', with: 'Jan Współautor 1'
        fill_in 'Nazwisko', with: 'Testowy Współautor 1'
        choose('Mieszkaniec')
        fill_in 'Numer telefonu', with: '+48 789 654 321'
        fill_in 'Email', with: email_coauthor_1
        fill_in 'Ulica', with: 'Dewajtis'
        fill_in 'Numer domu', with: '3'
        fill_in 'Numer mieszkania', with: '4'
        fill_in 'Kod pocztowy', with: '01-815'
        fill_in 'Miasto', with: 'Warszawa'

        page.check('project[coauthor1_show_author_name]')
        page.check('project[coauthor1_inform_author_about_implementations]')
        page.check('project[coauthor1_email_on_notification]')
        page.check('project[signed_by_coauthor1]')
      end

      within('#coauthor-two-js') do
        fill_in 'Imię', with: 'Jan Współautor 2'
        fill_in 'Nazwisko', with: 'Testowy Współautor 2'
        choose('Mieszkaniec')
        fill_in 'Numer telefonu', with: '+48 777 666 555'
        fill_in 'Email', with: email_coauthor_2
        fill_in 'Ulica', with: 'Dewajtis'
        fill_in 'Numer domu', with: '3'
        fill_in 'Numer mieszkania', with: '5'
        fill_in 'Kod pocztowy', with: '01-815'
        fill_in 'Miasto', with: 'Warszawa'

        page.check('project[coauthor2_show_author_name]')
        page.check('project[coauthor2_inform_author_about_implementations]')
        page.check('project[coauthor2_email_on_notification]')
        page.check('project[signed_by_coauthor2]')
      end
