...
module Decidim
  RSpec.describe 'submit project as koordynator diff scope', type: %i[feature mailer] do
    ...
    let!(:user) do
      create(
        ...
        ad_role: :decidim_bo_zol_koord
      )
    end

    it 'redirects to create project page with flash message due to inappropriate role (different scope)' do
      sign_in(user)
      visit url_target
      ...
      click_button 'Utwórz'
      expect(page).to have_content('Nie masz uprawnień do wykonania tej czynności')
      expect { Decidim::Projects::Project }.not_to change(Decidim::Projects::Project, :count)
    end
  end
end
