...
Decidim::Comments::Comment.class_eval do
  ...
  default_scope -> { joins('JOIN decidim_users ON decidim_users.id = decidim_comments_comments.decidim_author_id').joins('JOIN decidim_projects_projects ON decidim_projects_projects.id = decidim_comments_comments.decidim_root_commentable_id') }
